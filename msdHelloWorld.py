...
#empty list to store songids
songNameAndArtist = []
#function created by harpreetsingh. Getting songNameAndArtist for all tracks
def func_to_get_songNameAndArtist(filename):
    """
    This function does 3 simple things:
    - open the song file
    - get mbid and put it
    ...
    """
    h5 = GETTERS.open_h5_file_read(filename)
    songTemp = GETTERS.get_title(h5)
    artistTemp = GETTERS.get_artist_name(h5)
    songNameAndArtist.append(tuple((songTemp,artistTemp)))
    h5.close()

#apply_to_all_files(msd_subset_data_path,func=func_to_get_songNameAndArtist)

#print songNameAndArtist[0],songNameAndArtist[1]
energyValues = np.array([])
hotnessValues = np.array([])
tempoValues = np.array([])
danceabilityValues = np.array([])
#returnMedianValues for energy, speechiness, tempo, danceability
def returnMedianValues(filename):
    global energyValues, hotnessValues, tempoValues, danceabilityValues
    h5 = GETTERS.open_h5_file_read(filename) #this stores the file in h5 variable
    #songEnergy = GETTERS.get_energy(h5)
    songHotness = GETTERS.get_song_hotttnesss(h5)
    songTempo = GETTERS.get_tempo(h5)
    songDanceability = GETTERS.get_danceability(h5)
    #energyValues.append(songEnergy)
    if not(songHotness > 0.0):
        hotnessValues = np.append(hotnessValues, 0.0)
    else:
        hotnessValues = np.append(hotnessValues, songHotness)
    tempoValues = np.append(tempoValues,songTempo)
    danceabilityValues = np.append(danceabilityValue,songDanceability)
    h5.close()

apply_to_all_files(msd_subset_data_path,func=returnMedianValues)

#energyValues = np.array(energyValues)
#hotnessValues = np.array(hotnessValues)
#tempoValues = np.array(tempoValues)
#danceabilityValues = np.array(danceabilityValues)

#print "energyValues median is "+ str(np.median(hotnessValues))
print tempoValues[:40]
